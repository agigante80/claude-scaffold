...
### 1. Verify claude-scaffold is available
...
### 2. Show recent claude-scaffold changes
...
### 4. Inventory the current project
...
- **Report only - never auto-apply.** Always list what should change, never change files.
- **Skip upgrade-audit itself** when comparing skills (it's in the reference but that's fine).
- **Ignore Arivit-specific agents** that are not in claude-scaffold (safety-logic-reviewer,
  prisma-schema-guardian, etc.) - they are intentionally project-specific.
